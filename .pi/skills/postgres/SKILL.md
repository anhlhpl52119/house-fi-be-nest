...
## Connect to my postgres in local
**If you wanna connect to local database to query, test, execute, use `psql` cli installed my app with following database URL "host=localhost user=postgres password=postgres dbname=postgres port=5432 sslmode=disable"**

e.g

```bash
 psql "host=localhost user=postgres password=postgres dbname=postgres port=5432 sslmode=disable" -c "SELECT * FROM users LIMIT 10;"
```
